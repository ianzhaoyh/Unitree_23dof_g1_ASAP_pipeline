...
if __name__ == "__main__":
    ...
    policy_path = "/home/ianzhao/ASAP23dof//deploy/deploy_mujoco/configs/exported/model_2000.onnx"
    ...
    xml_path = "/home/ianzhao/ASAP23dof/deploy/deploy_mujoco/configs/g1_23dof.xml"
    ...
    #                             0.0, 
    #                             0.24, 0.28, 0.0, 0.81, 
    #                              0.25, -0.28, 0.0, 0.77 ], dtype=np.float32)
        
    # arm_waist_target : [0.0, 0,0,0.0,
#                    0.24, 0.28, -0.07, 0.81,0.0,0.0,0.0,
#                    0.25, -0.28, 0.08, 0.77,0.0,0.0,0.0]
    # kps = np.array([ 100, 100, 100, 200, 40, 40, 
    #                  100, 100, 100, 200, 40, 40, 
    #                  400, 
    #                  90,   60,  20, 60, 
...
# Isaac Gym KPandKD
    kps = np.array([ 100, 100, 100, 150, 40, 40, 
                     ...
                     400, 
                     100,   100,  50, 50, 
                     100,   100,  50, 50 ], dtype=np.float32)
    kds = np.array([ 2.0, 2.0, 2.0, 4.0, 0.2 , 0.2, 
                     2.0, 2.0, 2.0, 4.0, 0.2, 0.2, 
                     5.0, 
                     2.0, 2.0, 2.0, 2.0, 
    ...
    goal_target_dof_pos = default_angles.copy()
    current_target_dof_pos = default_angles.copy()
    ref_motion_phase = 0

    interpolation_alpha = 0.05 #加上这个 不会倒了
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        while viewer.is_running() and time.time() - start < simulation_duration:
            ...
            current_target_dof_pos += interpolation_alpha * (goal_target_dof_pos - current_target_dof_pos)
            tau = pd_control(current_target_dof_pos, d.qpos[7:], kps, np.zeros_like(kds), d.qvel[6:], kds)
            # current_target_dof_pos += interpolation_alpha * (goal_target_dof_pos - current_target_dof_pos)
            ...
            if counter % control_decimation == 0:
                ...
                goal_target_dof_pos = action * 0.25 + default_angles

                goal_target_dof_pos = np.clip(goal_target_dof_pos, dof_pos_limit_low, dof_pos_limit_up)
